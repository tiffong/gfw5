...
###PLOTTING####-----
name = "4 MPAs"
title = paste('../Figures/monthly_scatterplot/monthly_cumulative/', name, '.png', sep = '')
...
ggplot(Rdata[[1]], aes(x=c(-12:11), y=fishing_effort, group=1)) + 
  ...
  geom_line(data=Rdata[[2]], (aes(color="Pacific Remote Islands")), size = thick) +
  geom_line(data=Rdata[[3]], (aes(color="Papahānaumokuākea")), size = thick) +
  geom_line(data=Rdata[[4]], (aes(color="Phoenix Islands Protected Area")), size = thick) +
  geom_line(data=Rdata[[5]], (aes(color="Pitcairn")), size = thick)  +
  geom_line(data=Rdata[[6]], (aes(color="Ross Sea")), size = thick)  +
