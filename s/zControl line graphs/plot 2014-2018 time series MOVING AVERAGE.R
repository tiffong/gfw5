...
quartz()
#plots[[1]] =  
  ggplot(Rdata[[6]], aes(x=months_o_year, y=x, group=1)) + 
...
#MIDDLE FISHING RUNNING AVERAGE------

#name = 'MIDDLE_Avg'
#title = paste('../Figures/CONTROL_EEZs_4YEAR/eezs_plotted_together/Running_Avg/', name, '.png', sep = '')
#png(title, units="in", width=8, height=3, res=300)

#quartz()
plots[[2]] = ggplot(Rdata.avg[[6]], aes(x=months_o_year, y=x, group=1)) + 
geom_line(data=Rdata.avg[[5]], (aes(color=legend[[5]])), size = thick) +
geom_line(data=Rdata.avg[[15]], (aes(color=legend[[15]])), size = thick) +
geom_line(data=Rdata.avg[[21]], (aes(color='US (Hawaii)')), size = thick) +
geom_line(data=Rdata.avg[[11]], (aes(color=legend[[11]])), size = thick) +
  
  theme_minimal() +
  #geom_text(data = df, (aes(x=x2, y=yval_med)), label= names, hjust=1,vjust=1.5, check_overlap = TRUE, angle=30, size = 3) +
  geom_point(data = df,(aes(x=x, y=yval_med)),group= 1, shape = 21, colour = "black", fill = "white", size=5) +
...
#quartz()
plots[[3]] = ggplot(Rdata.avg[[6]], aes(x=months_o_year, y=x, group=1)) + 
  theme_minimal() +
  geom_line(data=Rdata.avg[[14]], (aes(color=legend[[14]])), size = thick) +
  geom_line(data=Rdata.avg[[20]], (aes(color=legend[[20]])), size = thick) +
  geom_line(data=Rdata.avg[[17]], (aes(color=legend[[17]])), size = thick) +
  geom_line(data=Rdata.avg[[22]], (aes(color=legend[[22]])), size = thick) +
  geom_line(data=Rdata.avg[[7]], (aes(color='Kiribati (Gilbert)')), size = thick) +
  ...
  theme(plot.title = element_text(hjust = 0.5)) + #this does nothing
  #geom_text(data = df, (aes(x=x2, y=yval_large)), label= names, hjust=1,vjust=1.5, angle=30, size = 3) +
  geom_point(data = df,(aes(x=x, y=yval_large)),group= 1, shape = 21, colour = "black", fill = "white", size=5) +
...
#grid arrange --------
name_plot = '3 plots'
title = paste('../Figures/CONTROL_EEZs_4YEAR/eezs_plotted_together/Running_Avg/', name_plot, '.png', sep = '')
title
png(title, units="in", width=13, height=16, res=300)
...
#SHARK SANCTUARY RUNNING AVERAGE--------
...
ggplot(Rdata.avg[[6]], aes(x=months_o_year, y=x, group=1)) + 
  theme_minimal() +
  geom_line(data=Rdata.avg[[3]], (aes(color=legend[[3]])), size = thick) +
  geom_line(data=Rdata.avg[[6]], (aes(color=legend[[6]])), size = thick) +
  geom_line(data=Rdata.avg[[8]], (aes(color=legend[[8]])), size = thick) +
  geom_line(data=Rdata.avg[[9]], (aes(color=legend[[9]])), size = thick) +
  geom_line(data=Rdata.avg[[10]], (aes(color=legend[[10]])), size = thick) +
  geom_line(data=Rdata.avg[[12]], (aes(color=legend[[12]])), size = thick) +
  geom_line(data=Rdata.avg[[13]], (aes(color=legend[[13]])), size = thick) +
  geom_line(data=Rdata.avg[[18]], (aes(color=legend[[18]])), size = thick) +
