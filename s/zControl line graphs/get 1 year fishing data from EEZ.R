...
#setwd('/Users/timwhite/gfw3/s') # changed for Tim's computer
library(sf)
library(mregions)
library(DBI)
library(bigrquery)
...
# Read me-------------------
#Using the year before and year after dates of an MPA, finds surrounding EEZs and gets data on monthly fishing effort
#for those EEZs. Was put in a loop, but loop isn't working.
...
eezs = c('Tuvaluan Exclusive Economic Zone', 
...
#for Nazca
eezs = c('Chilean Exclusive Economic Zone',
         'Peruvian Exclusive Economic Zone')

#TODO: set mpa name/creation date/mpant ------
###########VISUALIZING THE MPA#############
creation_date = '2014-09-25'
mpa_name = 'Pacific Remote Islands'
  ...
  #2 YEARS TIME
  #year_before_date = as.Date('2014-09-16')
  #year_after_date = as.Date('2016-09-16')
...
######MAKING THE DATA FRAME - done a very janky way -----------
  ...
  #GET 24 MONTHS FOR X AXIS of plot
...
##############SAVING TO AN R DATA FILE#########################
  ...
  #file_name = paste("../data/monthly_effort_CONTROL_4YEAR/", geoname, ".Rdata", sep = '')
...
#NOTE: plotting is moved to another file
