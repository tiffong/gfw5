...
quartz()
grid.arrange(effortPlot[[1]], effortPlot[[2]], effortPlot[[3]], nrow=1)
dev.off()
...
yo = binned_effort[[1]]
for(i in 1:nrow(yo)) {
  if (yo$lon_bin_center[i] >0) {
    yo$lon_bin_center[i] = yo$lon_bin_center[i] - 350
  } else {
    yo$lon_bin_center[i] = yo$lon_bin_center[i] + 10
  }
}
...
#myworld = maps::map(wrap=c(150,170), plot=FALSE, fill=TRUE)

#this itself gets hawaii fully on a pacific centered map
ggplot() + 
...
# +
#   coord_f(1.3) + coord_equal(xlim = c(0, 360), ylim = c(-90, 90)) 

###trying my own ggplot
  ...
  ggplot() +
  geom_raster(aes(x = lon_bin_center, y = lat_bin_center, fill = fishing_hours))+
  viridis::scale_fill_viridis(name = "Effort")
...
# Example
  ...
  z= LongLatToUTM(x,y,4)
  
  #not working
  hi = LongLatToUTM(binned_effort[[1]]$lat_bin_center,binned_effort[[1]]$lon_bin_center,4)
  
################  
###### another potential soln
  
xy <- data.frame(fishing_hours = 1:2, lat_bin_center = c(118, 119), lon_bin_center = c(10, 50))
coordinates(xy) <- c("lat_bin_center", "lon_bin_center")
proj4string(xy) <- CRS("+proj=utm")  ## for example
